...
export function GameCard({ game }: { game: Game }) {
  ...
  const slotsLeft = Math.max(0, game.total_slots - game.filled_slots);
  const endingSoon = new Date(game.ends_at).getTime() - Date.now() < 1000 * 60 * 60; // < 1h
  const almostFull = pct >= 80;
  const hot = game.featured || endingSoon || almostFull;

  return (
    <motion.div whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }} className="relative">
      {hot && <div className="absolute -inset-[2px] rounded-3xl pulse-gradient opacity-70 blur-[2px] -z-10" />}
      <div className={`bg-gradient-card rounded-3xl overflow-hidden shadow-card border border-border ${hot ? "glow-border" : ""}`}>
        <Link to="/games/$id" params={{ id: game.id }} className="block">
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            {!game.featured && almostFull && (
              <span className="absolute top-3 left-3 inline-flex items-center gap-1 bg-destructive text-destructive-foreground text-[10px] font-bold px-2 py-1 rounded-full">
                🔥 Only {slotsLeft} left
              </span>
            )}
            <span className={`absolute top-3 right-3 glass text-xs font-mono px-2 py-1 rounded-full flex items-center gap-1 ${endingSoon ? "text-destructive" : ""}`}>
              <Clock className={`h-3 w-3 ${endingSoon ? "fire-fx text-destructive" : "text-primary"}`} />
              <span className={endingSoon ? "count-pulse" : ""}>{cd}</span>
            </span>
          </div>
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1">
              <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                <div className={`h-full ${almostFull ? "pulse-gradient" : "bg-gradient-primary"}`} style={{ width: `${pct}%` }} />
              </div>
